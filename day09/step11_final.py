...
# img_ori = cv2.imread('./img/01가0785.jpg')
...
for i, plate_img in enumerate(plate_imgs):
    ...
    ocr_data = pytesseract.image_to_data(img_result_inverted, lang='kor', 
                                         config='--psm 7 --oem 1', 
                                         output_type=pytesseract.Output.DICT)
    
    # 신뢰도 점수(conf) 추출 (Tesseract는 -1을 '인식 불가'로 표시하므로 필터링)
    print(f"\n--- [상세 신뢰도 리포트] ---")
    for tc in range(len(ocr_data['text'])):
        text = ocr_data['text'][i].strip()
        conf = int(ocr_data['conf'][i])
        
        if text != "" and conf != -1: # 의미 없는 빈칸 제외
            print(f"글자(덩어리): '{text}' -> 신뢰도: {conf}%")

    conf_values = [int(conf) for conf in ocr_data['conf'] if int(conf) != -1]

    if len(conf_values) > 0:
        avg_conf = np.mean(conf_values) # 전체 글자의 평균 신뢰도
    else:
        avg_conf = 0

    # 기존 문자열 추출 로직 (결과값만 가져오기)
    chars = ocr_data['text']
    chars = "".join(chars).strip()
    
    print(f"[결과] {chars} (신뢰도: {avg_conf:.2f}%)")
...
ext = '.jpg'
result, encoded = cv2.imencode(ext, img_out)
if result:
    with open(chars + ext, 'wb') as f:
